...
def generate3Dplot(data, title="", axis_labels=("X", "Y", "Z"), axis_lim = ([-10, 15], [-10, 15], [-10, 15])):
    ...
    ax.set_zlabel(axis_labels[2], rotation=0)
    ax.set_title(title, loc='center')
    return plt.show()

def generate2Dplot(data, title="", axis_labels=("X", "Y", "Z"), axis_lim_inc = True, axis_lim = ([-10, 10], [-10, 10])):
    fig = plt.figure(figsize=(13, 9))
    ax = fig.add_subplot()
    ax.plot(data[:, 0], data[:, 1], 'blue')
    if axis_lim_inc:
        ax.set_xlim(axis_lim[0][0], axis_lim[0][1])
        ax.set_ylim(axis_lim[1][0], axis_lim[1][1])
    ax.set_xlabel(axis_labels[0], rotation=0)
    ax.set_ylabel(axis_labels[1], rotation=90)
    ax.set_title(title, loc='center')
    return plt.show()